...
public class CRemoteVariableTranspiler {
	...
	public static boolean transpile(String strinput, String stroutput) {
		...
		CArray csv = _.carray();
		csv.push(_.a("Type", "Character"));
		for(int i=0; i<tt.length(); i++)
			csv.push(_.a(tt._chash(i)._string("m_strtype"), tt._chash(i)._string("m_strtoken")));
		_.save_csv_file("C:/Users/developer/Desktop/st-final-project/tokentypes.csv", csv);
		
		// create the compiler object
		if(ccompiler.create(_.file_get_contents(strinput)) == false)
			return false;
		...
		csv.push(_.a("Index", "Type", "Character"));
		for(int i=0; i<t.length(); i++) {
			...
			if(str.equals(""))
				continue;
			csv.push(_.a(i, str, token._string("m_strname").replace(",","comma".trim())));
		} // end for
		...
		if(cparser.parse(_.cfunction("BODY")) == null)
			return false;
		
		CNode root = cparser.getParseTree();
		String ast = CRemoteVariableTranspiler.buildAST(root);
		_.file_set_contents("C:/Users/developer/Desktop/st-final-project/ast.txt",ast);
		
		root = cparser.getParseTree();
		ast = CRemoteVariableTranspiler.buildTAST(root);
		_.file_set_contents("C:/Users/developer/Desktop/st-final-project/translated-ast.txt",ast);
		
		// translate the file and write output to current directory
		return _.file_set_contents(stroutput, cparser.translate());
	} // end transpile()
	...
	public static boolean addNonTokensFunctions() {		
		...
		CFunction BODY = new CFunction("BODY") {
			public CReturn call(CArray args) {
				...
				while(true) {	
					...
					CNode cnode = cparser.acceptNonToken(_.cfunction("REMOTE_CREATE")); // REMOTE_CREATE
					if(cnode != null) { 
						pnode.add(cnode);
						continue;
					} // end if
					
					cnode = cparser.acceptNonToken(_.cfunction("REMOTE_DELETE")); // REMOTE_DELETE
					if(cnode != null) { 
						pnode.add(cnode);
						continue;
					} // end if
					
					cnode = cparser.acceptNonToken(_.cfunction("REMOTE_READ_UPDATE")); // REMOTE_READ_UPDATE
					if(cnode != null) { 
						pnode.add(cnode);
						continue;
					} // end if
		...
		CFunction REMOTE_CREATE = new CFunction("REMOTE_CREATE") { 
			public CReturn call(CArray args) {
				...
				CNode cnode = cparser.acceptNonToken(_.cfunction("PARAMS"), ">", false); // BODY - terminate at last ) token			
				if(cnode != null)
					pnode.add(cnode);
				...
				cnode = cparser.acceptNonToken(_.cfunction("IDENTIFIER"), _.args(")", "\n", "\t", ";"), false); // BODY - terminate at last } token			
				if(cnode == null)
					return CReturn._done(null);
		...
		CFunction REMOTE_READ_UPDATE = new CFunction("REMOTE_READ_UPDATE") { 
			public CReturn call(CArray args) {
				...
				if(cparser.acceptNonToken(_.cfunction("IDENTIFIER"), _.args(")", ",", "\n", "\t", ";", "="), false) == null) // BODY - terminate at last } token			
					return CReturn._done(null);
		...
		CFunction REMOTE_DELETE = new CFunction("REMOTE_DELETE") { 
			public CReturn call(CArray args) {
				...
				CNode cnode = cparser.acceptNonToken(_.cfunction("IDENTIFIER"), _.args(")", ",", "\n", "\t", ";"), false); // BODY - terminate at last } token			
				if(cnode == null)
					return CReturn._done(null);
