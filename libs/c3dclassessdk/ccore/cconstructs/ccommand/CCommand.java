...
public class CCommand extends CFunction {
	...
	public CArray m_tmpfilename = _.carray();
	...
	public CCommand(String strname, String strscriptname) { 
		this.m_strname = _.check_file_path(strname);
		this.m_strscriptname = _.check_file_path(strscriptname);
	...
	public CReturn call(CArray args) { 
		...
		CReturn creturn = CReturn._done(_.exec_command(strargs));
		if(creturn.isdone()) {
			this.removeTmpFiles();
		} // end if
		return creturn;
	...
	public String argsToString(CArray args) {
		...
		int itime = (int)_.time();
		String strargs = this.m_strname + " " + this.m_strscriptname;
		for(int i=0; i<args.length(); i++) {
			if(args._is_cmatrix(i)) {
				String strfilename = _.get_home_path() + "/argfilename_" + ivid + "_" + i + "_" + itime;  
				...
				String strfilename = _.get_home_path() + "/argfilename_" + ivid + "_" + i + itime;  
				args._cmatrix(i).toFile(strfilename);
				args._(i, strfilename); // change arg to filename
				m_tmpfilename.push(strfilename);
			} // end if
			if(args._is_string(i) && (args._string(i).indexOf(" ") != -1 || args._string(i).indexOf("\t") != -1))  
				args._(i, _.s(args._string(i))); // change arg to filename		
		} // end for
