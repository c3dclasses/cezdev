...
public class CMemory extends CResource { 
	...
	public CReturn upsert(String strname, Object value, String strtype, CHash params) {
		...
		return (creturn == null || creturn._boolean() == false) ? this.update(strname, value, strtype, params) : creturn;
	} // end create_update()
	...
	public String toString() { return super.toJSON(false) + "\n" + this.m_cache.toJSON(false); }
	public CMemoryDriver driver() { return (CMemoryDriver) this._("cmemorydriver_object"); }
	...
	static public CResource include(String strid, String strconfigfilename, CHash params) {
		CHash config = (CHash) _.json_decode_from_file(strconfigfilename, true);	
		if(config == null) {
			CLog.error("CMemory.include() - No Configuration file was setup for " + strconfigfilename);
			CLog.error("CMemory.include() - Please create or check " + strconfigfilename);
			return null;
		} // end if
		String strpath = (String)config._("cmemory-path");
		String strdriver = (String)config._("cmemory-driver");
		return CMemory.include(strid, strpath, strdriver, params);
	} // end include()
