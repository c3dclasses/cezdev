...
if not "%LIBRARY%"=="" (
    call "%CEZDEV%\cplatform.crud.bat" %ACTION% "%_CLIBRARIES%" "%LIBRARY%"
    exit /b 0
)
...
for /d %%P in ("%_CLIBRARIES%\*") do (
    if /i not "%%~nP"=="cboot" (
        echo Creating: %%~nP
        call "%CEZDEV%\cplatform.crud.bat" %ACTION% "%_CLIBRARIES%" "%%~nP"
    )
)
...
if exist "%_CLIBRARIES%\cboot" (
    echo Creating: cboot
    call "%CEZDEV%\cplatform.crud.bat" %ACTION% "%_CLIBRARIES%" "cboot"
)
